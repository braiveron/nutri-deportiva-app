export default function StatusModal({ type, title, message, onClose, onConfirm }) {
  ...
  const isLoading = type === 'loading'; // 👇 Nuevo estado de carga

  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center px-4 animate-fade-in">
      {/* FONDO OSCURO (Bloqueado si está cargando) */}
      <div 
        className="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity"
        onClick={(!isConfirm && !isLoading) ? onClose : undefined} 
      ></div>
      ...
      <div className="relative bg-sportDark border-2 border-white/10 shadow-2xl rounded-2xl p-8 max-w-sm w-full text-center transform transition-all scale-100">
        ...
        <div className={`mx-auto flex items-center justify-center h-16 w-16 rounded-full mb-6 ${
            ...
            isLoading ? 'bg-blue-500/10 text-blue-400' : // Color para carga
            ...
            <>
                {isConfirm ? (
                    <div className="flex gap-3">
                        ...
                        <button
                            onClick={onConfirm}
                            className="flex-1 py-3 rounded-lg font-bold text-xs uppercase tracking-widest bg-red-600 hover:bg-red-500 text-white shadow-lg shadow-red-900/20 transition-transform transform hover:scale-105"
                        >
                            Sí, Cancelar
                        </button>
                    </div>
                    ...
                    className={`w-full py-3 px-4 rounded-lg font-bold text-sm uppercase tracking-widest transition-all transform hover:scale-[1.02] shadow-lg ${
                        ...
                        : 'bg-sportRed hover:bg-red-600 text-white shadow-red-900/20'
                    }`}
                    >
